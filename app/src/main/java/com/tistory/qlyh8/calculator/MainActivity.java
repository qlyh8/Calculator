...
public class MainActivity extends AppCompatActivity {
    ...
    public void resultInit(){
        result = zero;    // 결과값 초기화
        fractionResult[0] = zero;    // 분수(분자) 결과값 초기화
        fractionResult[1] = zero;    // 분수(분모) 결과값 초기화
        fractionResult[2] = zero;    // 분수(대분수) 결과값 초기화
    ...
    public void clickBtnNumber(View view) {
        ...
        if(calcUtils.isSymbol(lastValue)){
            ...
            if(lastValue.equals(zero)){
            ...
            else if(lastValue.equals(plusMinus+zero)){
                ...
                if(lastValue.contains(fraction)){ // 분수일 때, 분자에 값을 넣는다.
                    if(!lastStrOfLastVal.equals(fraction) && lastValue.split(fraction)[1].equals(zero)) {
                        ...
                        arrayList.set(arrayList.size()-1,
                                lastValue.substring(0, lastValue.length()-1) + String.valueOf(view.getTag()));
                        viewUtils.changeTextView(arrayList, "calcFractionTopTextView",
                                minus + String.valueOf(view.getTag()));
                    }
                    else {
        ...
        if(!arrayList.isEmpty()){
            ...
            if(!calcUtils.isSymbol(lastValue) && !lastStrOfLastVal.equals(plusMinus)
                ...
                if(lastValue.contains(fraction)){
                    // "." 은 한 번만 들어가야 한다.
                    if(!lastValue.split(fraction)[1].contains(point)){
                        arrayList.set(arrayList.size()-1, lastValue + String.valueOf(view.getTag()));
                        viewUtils.appendTextView(arrayList, "calcFractionTopTextView", String.valueOf(view.getTag()));
                        viewUtils.changeFractionLine(arrayList);    // 분자가 분모보다 너비가 커지면 라인을 조정한다.
                    }
                } else{
                    if(!lastValue.contains(point)){ // "." 은 한 번만 들어가야 한다.
                        arrayList.set(arrayList.size()-1, lastValue + String.valueOf(view.getTag()));
                        viewUtils.appendTextView(arrayList, "calcTextView", String.valueOf(view.getTag()));
                    }
                }
                ...
                if(lastValue.contains(fraction)) {
                    ...
                    if(lastValue.substring(lastValue.length()-1).equals(fraction)){
                        ...
                        if(topValue.contains(plusMinus))
                            viewUtils.changeTextView(arrayList,"calcFractionTopTextView",
                                    topValue.substring(0, topValue.length()-1).replace(plusMinus, minus));
                        else
                    ...
                    arrayList.set(arrayList.size()-1, lastValue.substring(0, lastValue.length()-1));
                    if(lastValue.contains(plusMinus))
                        viewUtils.changeTextView(arrayList, "calcTextView",
                                lastValue.substring(0, lastValue.length()-1).replace(plusMinus, minus));
                    else
    ...
    public void clickBtnResult(View view) {
        ...
        if(!arrayList.isEmpty()) {
            ...
            if(!calcUtils.isSymbol(lastValue)
                    && !lastStrOfLastVal.equals(point)
                    && !lastStrOfLastVal.equals(fraction)
                ...
                try {
                    ...
                    fractionResult[0] = zero;
                    fractionResult[1] = zero;
                    fractionResult[2] = zero;
